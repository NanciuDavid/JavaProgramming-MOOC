...
public class RecipesBook {
    ...
    public void findRecipeByName(String recipeName) {
        List<Recipe> temp = recipes.stream().filter(recipe -> recipe.getName().contains(recipeName))
                .collect(Collectors.toList());
        temp.stream().forEach(System.out::println);
    }

    public void findRecipeByCookingTime(int cookingTime) {
        List<Recipe> temp = recipes.stream().filter(recipe -> recipe.getCookTime() <= cookingTime)
                .collect(Collectors.toList());
        temp.stream().forEach(System.out::println);
    }

    public void findRecipeByIngredients(String ingredients) {
        List<Recipe> temp = recipes.stream().filter(recipe -> recipe.getIngredients().contains(ingredients))
                .collect(Collectors.toList());
        temp.stream().forEach(System.out::println);
    }
}